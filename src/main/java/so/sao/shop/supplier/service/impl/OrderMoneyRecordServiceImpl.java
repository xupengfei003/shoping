...
public class OrderMoneyRecordServiceImpl implements OrderMoneyRecordService {
	...
	public Map<String, Object> saveOrderMoneyRecord(Long userId) {
		...
		if (currentNum < Constant.OMRTimePeriodConfig.START_DATE
				|| currentNum > Constant.OMRTimePeriodConfig.END_DATE) {
		...
		if (null == account) {
			...
			omr.setState("0");// 状态
			omr.setCreatedAt(new Date());// 创建时间
			omr.setUpdatedAt(new Date());// 更新时间
	...
	private List<OrderMoneyRecordVo> convertOrderMoneyRecordVo(List<OrderMoneyRecord> list) {
		...
		if (null != list && !list.isEmpty()) {
			for (OrderMoneyRecord omr : list) {
				...
				ormVo.setCreatedAt(omr.getCreatedAt());
				ormVo.setUpdatedAt(omr.getUpdatedAt());
	 ...
	 * 返回开发规定的提现申请记录的Output对象 ①、利用PageHelper开启分页； ②、根据分页信息查询提现申请的
	 * List<OrderMoneyRecord>,并取得OrderMoneyRecord的分页信息；
	 ...
	 */
	@Override
	public OrderMoneyRecordOutput searchOrderMoneyRecords(Long userId, OrderMoneyRecordInput put, Integer pageNum,
			Integer pageSize) {
		// 创建提现申请记录的Output对象；
	...
	public List<PurchaseVo> convertPurchaseVo(List<Purchase> list) {
		...
		for (Purchase purchase : list) {
			...
			purchaseVo.setOrderCreateTime(purchase.getOrderCreateTime());
			purchaseVo.setOrderPaymentMethod(purchase.getOrderPaymentMethod());
			purchaseVo.setOrderPaymentTime(
					purchase.getOrderPaymentTime() == null ? null : purchase.getOrderPaymentTime());
